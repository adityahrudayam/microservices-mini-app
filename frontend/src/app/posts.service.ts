...
export class PostsService {
    ...
    getPostDetail(id: string): Observable<Post> {
        return this.http.get<{ message: string, post: any }>(`http://localhost:3000/posts/${id}`)
    ...
    editPost(id: string, title: string, content: string, image: File | string) {
        ...
        if (typeof image === 'object') { // new image upload case
            postData = new FormData();
            postData.append('id', id);
            postData.append('title', title);
            postData.append('content', content);
            postData.append('image', image, title);
        } else { // same image case
            postData = { id: id, title: title, content: content, imagePath: image };
        }
        return this.http.put<{ message: string, post: Post }>('http://localhost:3000/edit-post', postData)
            .subscribe((res) => {
    ...
    addPost(title: string, content: string, image: File) {
        ...
        return this.http.post<{ message: string, post: Post }>("http://localhost:3000/create-post", newPost)
    ...
    deletePost(postId: string) {
        return this.http.delete(`http://localhost:3000/delete-post/${postId}`)
